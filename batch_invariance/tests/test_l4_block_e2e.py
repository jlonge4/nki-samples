"""
L4 — Block E2E: prefix rows invariant under padded seq (continuous batching on block).

Uses demo dims (d_model=256) due to matmul SBUF N limit; nanochat per-op at 1280 is L1.
"""
...
def main() -> int:
    ...
    def test_block_prefix_packing() -> bool:
        print(f"\n--- L4: block prefix invariance (padded seq) device={get_device()} ---")
        seq = DEMO_SEQ
        d_model, d_ffn = DEMO_D_MODEL, DEMO_D_FFN
        ...
        print(f"  seq={seq} d_model={d_model}: {'PASS' if ok else 'FAIL'} max_abs={diff:.2e}")
        return bool(ok)

    def test_block_run_to_run(n_runs: int = 3) -> bool:
        print("\n--- L4: block run-to-run ---")
        weights = {k: to_neuron(v) for k, v in make_block_weights(DEMO_D_MODEL, 128, DEMO_D_FFN).items()}
    ...
    ok = test_block_prefix_packing() and test_block_run_to_run()
